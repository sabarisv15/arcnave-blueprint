...
- **Infrastructure**: PostgreSQL, Alembic migrations, RLS policies,
  file storage, structured logging, ConfigurationService schema,
  health checks
...
- `docs/api/` is generated by FastAPI from route definitions — never
  hand-written in advance of the code.
